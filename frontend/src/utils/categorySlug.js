...
function leafSlug(category) {
  ...
  return [category?.slug, category?.name, ...aliases].map(slugifyCategory).filter(Boolean)
}
...
export function resolveCategoryIdFromSlug(categories, slug) {
  const match = resolveCategoryFromList(categories, slug)
  if (!match) return null
  const id = categoryId(match)
  return id || null
}
