...
export function getGoogleOAuthRedirectUri(req) {
    ...
    // Dev: match how the browser reached the backend (localhost vs 127.0.0.1)
...
/** OAuth2 client; uses HTTPS_PROXY / GOOGLE_HTTPS_PROXY when Google APIs are unreachable directly. */
export function createGoogleOAuthClient(redirectUri) {
    ...
    if (proxyUrl) {
        console.log('[Google OAuth] Using HTTPS proxy:', proxyUrl.replace(/:[^:@/]+@/, ':***@'))
...
export function formatGoogleNetworkError(error) {
    ...
    if (
        /ETIMEDOUT|ECONNREFUSED|ENOTFOUND|ECONNRESET|EAI_AGAIN/i.test(msg) ||
        /oauth2\.googleapis\.com/i.test(msg)
    ) {
        const proxyHint = getGoogleProxyUrl()
            ? ''
            : ' 或在 backend/.env 设置 GOOGLE_HTTPS_PROXY=http://127.0.0.1:7890（改成你的代理地址）'
        return `无法连接 Google（oauth2.googleapis.com 超时）。请让运行后端的电脑能访问 Google（VPN/代理）${proxyHint}`
    }
